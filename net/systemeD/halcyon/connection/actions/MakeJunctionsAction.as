package net.systemeD.halcyon.connection.actions
{
     ...
     * cases. */
    public class MakeJunctionsAction extends CompositeUndoableAction
    {
        private var way: Way;
        private var connection: Connection;
        private var allWays: Array;
        private var onlyTransportTags: Boolean;
        private var _junctions: Array = new Array();

        public function MakeJunctionsAction(initWay: Way, initOnlyTransportTags: Boolean = true) {
            super('Make junctions')
            way = initWay;
            connection = way.connection;
            onlyTransportTags = initOnlyTransportTags;
        }

        /** Perform actions as they are generated. */
        ...
        public function getComputedJunctions(): Array {
        	// raise error if not computed?
        	return _junctions.slice();
        }
        ...
        protected function FindIntersectionsForSegment(p0: Point, p1: Point): Array {
            ...
            for each (var w:Way in allWays) {
                ...
                for (var i: uint = 1; i < w.length; i++) {
                    ...
                    // In my cursory investigation, some false negatives had rectangles of zero width. *shrug*
                    ...
                    if (true || qr.intersects(pr)) {
                        ...
                        if (z) {
                            ...
                            const epsilon2:Number = 0.0000001 // arbitrary definition of "near here" - square of actual distance (~20m)
                            var makejunction:Boolean = true;
                            var js:Array = way.getJunctionsWith(w);
                            // potential issue that we can create two very nearby junctions if our way
                            // crosses over an un-junctioned crossing. (ok, it's unlikely - and not that bad.)
                            for each (var n: Node in js/*.concat(junctions)*/) {
